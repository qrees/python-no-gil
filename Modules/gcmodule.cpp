...
/* Get an object's GC head */
#define AS_ACCGC(o) ((PyACCGC_Head *)(o)-1)

/* Get the object given the GC head */
#define FROM_ACCGC(g) ((PyObject *)(((PyACCGC_Head *)g)+1))
/*** Global GC state ***/
...
visit_decref(PyObject *op, void *data)
{
	...
	if (PyObject_IS_GC(op)) {
		...
		if (gc->gc.gc_refs > 0){
			//printf("INFO: %p Decreasing gc_refs from %i to %i\n", gc, gc->gc.gc_refs, gc->gc.gc_refs-1);
			gc->gc.gc_refs--;
		}
	}
	return 0;
...
subtract_refs(PyGC_Head *containers)
{
	...
	PyGC_Head *gc = containers->gc.gc_next;
	for (; gc != containers; gc=gc->gc.gc_next) {
		PyTypeObject* py_type = Py_TYPE(FROM_GC(gc));
		traverse = py_type->tp_traverse;
		if (traverse==0){
			//printf("ERROR: %p tp_traverse is NULL for obj:%p type:%s\n", gc, FROM_GC(gc), Py_TYPE(FROM_GC(gc))->tp_name );
			continue;
		}
		assert(traverse);
...
visit_reachable(PyObject *op, PyGC_Head *reachable)
{
	if (PyObject_IS_GC(op)) {
		...
		if (gc_refs == 0) {
			 ...
			 * reachable.
			 */
			//printf("INFO: %p Setting gc_refs to 1\n", gc);
			gc->gc.gc_refs = 1;
		}
			...
			//printf("INFO: %p Setting gc_refs to 1 and moving to reachable\n", gc);
			gc->gc.gc_refs = 1;
		}
...
move_unreachable(PyGC_Head *young, PyGC_Head *unreachable)
{
	...
	while (gc != young) {
		...
		if (gc->gc.gc_refs) {
                         ...
                         */
                        PyObject *op = FROM_GC(gc);
                        PyTypeObject* py_type = Py_TYPE(op);
			 ...
			 */
			next = gc->gc.gc_next;
			//printf("%p Moving to unreachable\n", gc);
...
visit_move(PyObject *op, PyGC_Head *tolist)
{
	if (PyObject_IS_GC(op)) {
		if (IS_TENTATIVELY_UNREACHABLE(op)) {
			PyGC_Head *gc = AS_GC(op);
			//printf("%p moving to new list, probably finalizers\n", gc);
...
{
	if (0 && PyInstance_Check(op)) {
		debug_instance(msg, (PyInstanceObject *)op);
	}else{
		//printf("Non instance object of type %s\n", op->ob_type->tp_name);
	}
	...
	while (!gc_list_is_empty(collectable)) {
		...
		if (debug & DEBUG_SAVEALL) {
			...
			if ((clear = Py_TYPE(op)->tp_clear) != NULL) {
				...
				//printf("WARN: cannon deallocate object of type %s\n", Py_TYPE(op)->tp_name);
			}
		}
...
collect_generations(void)
{
	...
	for (i = NUM_GENERATIONS-1; i >= 0; i--) {
		printf("Gneration %i: %i/%i\n", i, generations[i].count, generations[i].threshold);
...
gc_get_referents(PyObject *self, PyObject *args)
{
	...
	for (i = 0; i < PyTuple_GET_SIZE(args); i++) {
		...
		if (traverse==0){
			//printf("ERROR: gc, tp_traverse is NULL for gc:%p type:%s\n", AS_GC(obj), obj, py_type->tp_name );
			continue;
		}
...
void
PyObject_GC_Track_(void *op)
{
	//_PyObject_GC_TRACK(op);
	printf("Track object %p\n", op);
	PyGC_Head *g = ((PyGC_Head *)(op)-1);
	if (g->gc.gc_refs != (-2))
		Py_FatalError("GC object already tracked");
	g->gc.gc_refs = (-3);
	g->gc.gc_next = _PyGC_gray;
	g->gc.gc_prev = _PyGC_gray->gc.gc_prev;
	g->gc.gc_prev->gc.gc_next = g;
	_PyGC_gray->gc.gc_prev = g;
}
...
int move_to_list(PyObject *obj, void * list){
	...
	if(head->gc.gc_next){
		if(head->gc.color != &accgc_white){
			printf("ERROR: %p Moving object to gray list (%p, %p)\n", head, head->gc.gc_next, head->gc.gc_prev );
			gc_list_move(head, &accgc_gray);
			printf("ERROR: Object moved\n" );
		}
	}else{
...
void accgc_collect(){
	...
	while (frame){
		...
		if(head->gc.color != &accgc_gray){
			gc_list_move(head, &accgc_gray);
			head->gc.color = &accgc_gray;
		}
		frame = frame->f_back;
	}

	traverseproc traverse;
	PyGC_Head * curr = accgc_gray.gc.gc_next;
	PyGC_Head * next;
	while(curr != &accgc_gray){
		i++;
		...
		if (traverse!=0){
			...
			printf("ERROR: %p tp_traverse is NULL for gc:%p type:%s\n", curr, obj, obj->ob_type->tp_name );
		}
		
		next = curr->gc.gc_next;
		gc_list_move(curr, &accgc_black);
...
_PyObject_GC_Malloc(size_t basicsize)
{
	...
	//pthread_mutex_lock( &colored_mutex );
	...
	//list<PyGC_Head*> lista;
	//lista.push_back(g);
	//list<PyGC_Head*>::iterator curr = lista.begin();
	
	//g->gc.curr_node = curr._M_node;
	//g->gc.color = &accgc_white;
	//accgc_white.splice(accgc_white.end(), lista);
	
	g->gc.gc_next = &accgc_white;
	g->gc.gc_prev = accgc_white.gc.gc_prev;
	g->gc.gc_prev->gc.gc_next = g;
	accgc_white.gc.gc_prev = g;

	//list<PyGC_Head*>::reverse_iterator it = ++accgc_gray.rend();
	//printf("%p -> %p\n", g, *it);
	/*g->gc.gc_refs = (-3);
	g->gc.gc_next = _PyGC_gray;
	g->gc.gc_prev = _PyGC_gray->gc.gc_prev;
	...
	//pthread_mutex_unlock(&colored_mutex);
	pthread_spin_unlock(&colored_lock);
...
_PyObject_GC_Resize(PyVarObject *op, Py_ssize_t nitems)
{
	...
	g->gc.gc_prev = accgc_white.gc.gc_prev;
	g->gc.gc_prev->gc.gc_next = g;
	accgc_white.gc.gc_prev = g; // append to list again
